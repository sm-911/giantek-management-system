...
router.post('/', async (req, res) => {
  try {
    ...
    if (entryStatus === 'completed') {
      await db.execute('UPDATE work_entries SET completed_at = CURRENT_TIMESTAMP WHERE id = ?', [newEntryId]);
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (req.user.role === 'employee') {
      ...
      `, [entryId, req.user.id]);
      const isAssigned = assignedRows[0];
...
router.post('/:id/unlock', adminOnly, async (req, res) => {
  try {
    ...
    await db.execute('UPDATE work_entries SET is_locked = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [entryId]);
    ...
    const [assignedEmps] = await db.execute(`
      ...
      WHERE wee.work_entry_id = ?
    `, [entryId]);
    const [clientRows] = await db.execute('SELECT c.name FROM clients c JOIN work_entries we ON we.client_id = c.id WHERE we.id = ?', [entryId]);
